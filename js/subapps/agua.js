...
async function criarSessaoAtividade() {
  if (!aguaUser) return;
  const sessaoRef = db.collection('usuarios').doc(aguaUser.uid).collection('sessoes_sos').doc();
...
export function aguaRegistrarFissura(intensidade) {
  // Registra no Firestore
  saveFissura(intensidade);
...
async function saveFissura(intensidade) {
  ...
  // Atualizar atividade
  const ativRef = db.collection('usuarios').doc(aguaUser.uid)
  ...
  // Sessão
  await db.collection('usuarios').doc(aguaUser.uid)
    .collection('sessoes_sos').doc(aguaSessaoId)
