...
export let modoEdicao = false; // true quando editar perfil via engrenagem
...
export function navigateTo(screenId) {
  ...
  // Se for login e já tiver usuário, redireciona
  ...
  if (user) {
    ...
    try {
      ...
      if (doc.exists) {
        ...
        // Aplicar tema salvo no Firebase
        if (userProfile.tema) {
          aplicarTema(userProfile.tema, userProfile.ajusteImagem || 'cover');
        } else {
          carregarTemaSalvo(); // fallback para localStorage
        }
        ...
        iniciarOnboarding(false); // modo cadastro
      }
    } catch (e) {
      console.warn(e);
      if (!userProfile) {
...
// Config (engrenagem) – editar perfil
...
// Botão "Estou com vontade" – lista de estratégias
...
// Botão SOS emergência – respiração direta
document.getElementById('btn-sos-emergencia').addEventListener('click', () => {
  navigateTo('screen-respiracao');
  iniciarRespiracao(currentUser, userProfile, true); // true = modo SOS urgente
});

// Voltar das estratégias
document.getElementById('btn-strategies-back').addEventListener('click', () => {
  voltarEstrategias();
  navigateTo('screen-dashboard');
});

// Botão Sair do onboarding (cancela cadastro)
...
// Finalizar onboarding (Gerar plano)
...
document.getElementById('confirm-yes').addEventListener('click', async () => {
  ...
  if (strategyId) {
    await window._saveCravingLog(strategyId, 6, false, currentUser);
...
document.getElementById('confirm-no').addEventListener('click', async () => {
  ...
  const strategyId = window._selectedStrategyId;
  if (strategyId) {
    const strategyLabel = window._strategies.find(s => s.id === strategyId)?.label || strategyId;
    await window._registerCigarroAutomatico(`Fissura - não resistiu (${strategyLabel})`, currentUser);
    window._selectedStrategyId = null;
    navigateTo('screen-relapse');
  }
});

console.log('🚀 App inicializado com módulos!');
